...
void generate_ycsb_workload(dataset_t* dataset, kv_t** kv_ptrs, ycsb_workload* workload,
	...
	// std::random_shuffle(kv_ptrs + insert_offset, kv_ptrs + insert_offset + inserts_per_thread);
...
void test_ycsb(dataset_t* dataset, const ycsb_workload_spec* spec, unsigned int num_threads) {
	...
	std::sort(kv_ptrs, kv_ptrs + dataset->num_keys, compare);
...
int main(int argc, char** argv) {
	...
	num_threads = get_int_flag(args, "--threads", 1);
	...
	if (!strcmp(test_name, "read-heavy") || !strcmp(test_name, "mt-read-heavy")) {
		ycsb_spec = READ_HEAVY_SPEC;
		is_ycsb = 1;
	}
	if (!strcmp(test_name, "read-write-balanced") || !strcmp(test_name, "mt-read-write-balanced")) {
		ycsb_spec = READ_WRITE_BALANCED_SPEC;
		is_ycsb = 1;
	}
	if (!strcmp(test_name, "write-heavy") || !strcmp(test_name, "mt-write-heavy")) {
		ycsb_spec = WRITE_HEAVY_SPEC;
		is_ycsb = 1;
	}

	if (!strcmp(test_name, "write-only") || !strcmp(test_name, "mt-write-only")) {
