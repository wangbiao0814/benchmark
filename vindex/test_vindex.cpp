...
vIndexKV** read_kvs(dataset_t* dataset, uint64_t value_size) {
	uint64_t i;
	ct_key key;
	...
	for (i = 0;i < dataset->num_keys;i++) {
		...
		uint64_t pos = dynamic_buffer_extend(&kvs_buf, sizeof(vIndexKV) + key.size + value_size);
		vIndexKV* kv = (vIndexKV*) (kvs_buf.ptr + pos);
	...
	return (vIndexKV**) kv_ptrs;
}

uint8_t LCP(vIndexKV* kv[], int size)
{
	uint8_t lcp = kv[0]->keyLength;
	for(int i = 1; i < size; i++)
	{
		for(int j = 0; j < lcp; j++)
		{
			if(kv[0]->kv[j] != kv[i]->kv[j])
			{
				lcp = j;
				break;
			}
...
void mem_usage(char* dataset_name) {
	...
	// size_t frequency[16] = {0};
	// for(i=0;i<=dataset.num_keys-128;i++)
	// {
	// 	uint8_t lcp = LCP(kv_ptrs + i, 128);
	// 	for(int j = 1; j <= lcp; j++) frequency[j]++;
	// }

	// for(i =0; i<16; i++)
	// {
	// 	printf("%ld\t", frequency[i]);
	// }
	// printf("\n");
	// return;
	...
	// {
	// 	// printf("j=%d\n",j);
	// 	vIndexKV* tempKV = (vIndexKV*)GetvIndex(&vindex, kv_ptrs[j]);
	// 	if(tempKV != kv_ptrs[j])
	// 	{
...
void pos_lookup(dataset_t* dataset) {
	...
	for (i = 0;i < num_lookups;i++) {
		vIndexKV* tmpKV = (kv_ptrs[rand_uint64() % dataset->num_keys]);
		uint64_t data_size = sizeof(vIndexKV) + tmpKV->keyLength;
		uint64_t offset = dynamic_buffer_extend(&workloads_buf, data_size);
		vIndexKV* data = (vIndexKV*) (workloads_buf.ptr + offset);
	...
	clock_gettime(CLOCK_MONOTONIC, &start_time);
	for (i = 0;i < num_lookups;i++) {
		vIndexKV* targetKey = (vIndexKV*)buf_pos;
		GetvIndex(&vindex, targetKey);

		buf_pos += (sizeof(vIndexKV) + targetKey->keyLength);
		speculation_barrier();
	}
	clock_gettime(CLOCK_MONOTONIC, &end_time);
	notify_critical_section_end();
...
void* mt_lookup_thread(void* arg) {
	...
	for (i = 0; i < ctx->num_keys; i++) {
		vIndexKV* targetKey = (vIndexKV*)buf_pos;
		GetvIndex(ctx->vindex, targetKey);
		// if (kv == NULL) {
...
void mt_pos_lookup(char* dataset_name, unsigned int num_threads) {
	...
	for (i = 0; i < num_threads; i++) {
		...
		for (j = 0; j < lookups_per_thread; j++) {
			vIndexKV* kv = keys[rand_uint64() % dataset.num_keys];
			uint64_t data_size = sizeof(vIndexKV) + kv->keyLength;
			uint64_t offset = dynamic_buffer_extend(&workload_data, data_size);
			vIndexKV* data = (vIndexKV*) (workload_data.ptr + offset);
...
void* mt_insert_thread(void* arg) {
	...
	for (i = 0; i < ctx->num_keys; i++) {
		vIndexKV* kv = (vIndexKV*)buf_pos;
		InsertvIndex(ctx->vindex, kv);
		buf_pos += (sizeof(vIndexKV) + kv->keyLength + kv->valueLength);
		speculation_barrier();
	}
...
void execute_ycsb_workload(ycsb_thread_ctx* ctx) {
	...
	for (i = 0;i < ctx->workload.num_ops; i++) {
		...
		switch (op->type) {
			case YCSB_READ:{
				vIndexKV* key = (vIndexKV*) (ctx->workload.data_buf + op->data_pos);
				auto result = GetvIndex(index, key);
				if(result == NULL)
				{
					printf("Error: key not found\n");
					return;	
			...
			case YCSB_READ_LATEST:{
				...
				vIndexKV* key = (vIndexKV*) next_read_latest_key[inserter_idx];
				...
				if (key->keyLength != 0xFFFFFFFFU)
					next_read_latest_key[inserter_idx] += sizeof(vIndexKV) + key->keyLength;
			...
			case YCSB_UPDATE:{
				vIndexKV* updated_kv = (vIndexKV*) (ctx->workload.data_buf + op->data_pos);
				InsertvIndex(index, updated_kv);
				speculation_barrier();
			}
			break;

			case YCSB_INSERT:{
				vIndexKV* kv = (vIndexKV*) (ctx->workload.data_buf + op->data_pos);
				InsertvIndex(index, kv);
			...
			case YCSB_RMW:{
				vIndexKV* kv = (vIndexKV*) (ctx->workload.data_buf + op->data_pos);
				// Find existing value
				auto result = GetvIndex(index, kv);
				if (result == NULL) {
...
void generate_ycsb_workload(dataset_t* dataset, vIndexKV** kvs, ycsb_workload* workload,
	...
	for (i = 0; i < spec->num_ops; i++) {
		...
		switch (op->type) {
			...
			case YCSB_READ:{
				...
				data_size = sizeof(vIndexKV) + kv->keyLength;
				op->data_pos = dynamic_buffer_extend(&workload_buf, data_size);
				vIndexKV* target_key = (vIndexKV*) (workload_buf.ptr + op->data_pos);
			...
			case YCSB_UPDATE:{
				kv = kvs[rand_dist(&dist)];
				data_size = sizeof(vIndexKV) + kv->keyLength + kv->valueLength;
				op->data_pos = dynamic_buffer_extend(&workload_buf, data_size);

				vIndexKV* newKV = (vIndexKV*) (workload_buf.ptr + op->data_pos);
	...
	for (thread = 0; thread < num_threads; thread++) {
		...
		for (block = 0; block < num_inserts + 1; block++) {
			for (i = 0; i < read_latest_block_size; i++) {
				...
				vIndexKV* key = (vIndexKV*) (workload_buf.ptr + data_pos);
			...
			uint64_t sentinel_pos = dynamic_buffer_extend(&workload_buf, sizeof(vIndexKV));
			vIndexKV* sentinel = (vIndexKV*) (workload_buf.ptr + sentinel_pos);
...
bool compare(vIndexKV* kv1, vIndexKV* kv2)
{
...
void ycsb(char* dataset_name, const ycsb_workload_spec* spec) {
	...
	notify_critical_section_start();
	clock_gettime(CLOCK_MONOTONIC, &start_time);
	execute_ycsb_workload<vIndex*>(&ctx); 
	clock_gettime(CLOCK_MONOTONIC, &end_time);
	notify_critical_section_end();
	float time_took = time_diff(&end_time, &start_time);
...
void mt_ycsb(char* dataset_name, const ycsb_workload_spec* spec, unsigned int num_threads) {
	...
	for (i = 0;i < num_threads;i++) {
		...
		generate_ycsb_workload(&dataset, kv_ptrs, &(ctx->workload), spec, i, num_threads);
		ctx->index = &vindex;
		ctx->thread_id = i;
		ctx->inserts_done = 0;
